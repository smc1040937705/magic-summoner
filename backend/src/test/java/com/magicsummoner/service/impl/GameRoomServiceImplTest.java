...
/**
 * GameRoomServiceImpl 单元测试
 * 测试 GameRoomService 的所有公共方法
 */
...
class GameRoomServiceImplTest {
    ...
    @Test
    @DisplayName("根据ID获取房间 - 房间不存在 - 抛出异常")
    ...
    @Test
    @DisplayName("根据房间代码获取房间 - 房间不存在 - 抛出异常")
    ...
    void getRoomsByStatus_Success() {
        ...
        assertNotNull(result);
        assertEquals(1, result.size());
        assertEquals(GameRoomStatus.WAITING, result.get(0).getStatus());
    }
    ...
    @Test
    @DisplayName("加入房间 - 房间不在等待状态 - 抛出异常")
    ...
    @Test
    @DisplayName("通过代码加入房间 - 房间代码不存在 - 抛出异常")
    ...
    @Test
    @DisplayName("结束游戏 - 房间不存在 - 抛出异常")
    void endGame_RoomNotFound_ThrowsException() {
        // Given
        when(gameRoomRepository.findById(999L)).thenReturn(Optional.empty());

        // When & Then
        RuntimeException exception = assertThrows(RuntimeException.class,
                () -> gameRoomService.endGame(999L, 1L));
        assertEquals("Room not found", exception.getMessage());
    }
}
