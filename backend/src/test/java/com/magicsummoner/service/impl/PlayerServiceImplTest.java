package com.magicsummoner.service.impl;
...
/**
 * PlayerServiceImpl 单元测试
 * 测试 PlayerService 的所有公共方法
 */
...
class PlayerServiceImplTest {
    ...
    void getPlayerById_Success() {
        ...
        verify(dtoMapper).toPlayerDTO(testPlayer);
    }

    @Test
    @DisplayName("根据ID获取玩家 - 玩家不存在 - 抛出异常")
    ...
    @Test
    @DisplayName("根据用户名获取玩家 - 玩家不存在 - 抛出异常")
    ...
    @Test
    @DisplayName("获取所有玩家 - 空列表")
    void getAllPlayers_Empty() {
        // Given
    ...
    @Test
    @DisplayName("更新玩家信息 - 玩家不存在 - 抛出异常")
    void updatePlayer_NotFound_ThrowsException() {
        // Given
        PlayerDTO updateDTO = PlayerDTO.builder().displayName("New Name").build();
        when(playerRepository.findById(999L)).thenReturn(Optional.empty());
    ...
    @Test
    @DisplayName("增加金币 - 玩家不存在 - 抛出异常")
    void addGold_PlayerNotFound_ThrowsException() {
        // Given
        when(playerRepository.findById(999L)).thenReturn(Optional.empty());

        // When & Then
        RuntimeException exception = assertThrows(RuntimeException.class,
                () -> playerService.addGold(999L, 100));
        assertEquals("Player not found", exception.getMessage());
    }
    ...
    void addExperience_Success() {
        ...
        // When
        playerService.addExperience(1L, 200);
    ...
    @Test
    @DisplayName("更新排位分 - 正常场景 - 增加分数")
    void updateRankPoints_Increase() {
        // Given
        when(playerRepository.findById(1L)).thenReturn(Optional.of(testPlayer));
        when(playerRepository.save(any(Player.class))).thenReturn(testPlayer);
    ...
    @Test
    @DisplayName("获取玩家排名 - 第一名")
    void getPlayerRank_FirstPlace() {
        // Given
        when(playerRepository.findById(1L)).thenReturn(Optional.of(testPlayer));
        when(playerRepository.countPlayersWithHigherRank(1500)).thenReturn(0L);
